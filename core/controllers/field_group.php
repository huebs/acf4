...
class acf_field_group
{
	...
	*/
	...
	function __construct()
	{
		...
		// Stoping the plug-in from using field_# key values which don't tell us anything about
		//		the data they are storing.
		//
		// add_filter('acf/field_group/get_next_field_id', array($this, 'get_next_field_id'), 5, 1);
		//
		// Instead, keys will be prefaced with the namespace 'acf_' followed by the key's name and a
		// 		number if the key is already taken.

		add_filter('acf/field_group/find_available_key', array($this, 'find_available_key'), 5, 1);
	...
	function save_post($post_id)
	{
		...
		*/
		...
		if( $_POST['fields'] )
		{
			...
			foreach( $_POST['fields'] as $field )
			{
				...
				if( $field['key'] == 'field_clone' )
				{
					...
					$new_key = 'acf_' . $field['name']; 									// I don't like that key and the name don't match. Need to figure out way to deal with duplicates though.

					$field['key'] = apply_filters( 'find_available_key', $new_key );		// Find a version of the key that is available and assign it to the $field

				} else {		// If key is set to old style, update it.

					$legacy_key = $field['key']; // Store the old key for safe keeping

					$new_key = 'acf_' . $field['name']; // Set up the new key value.

					$field['key'] = find_available_key( $new_key ); // Find a version of the key that is available and assign it to the $field

					$field['legacy_key'] = $legacy_key; // Keeping the legacy key arround for good measure.
				}
		...
		foreach( $keys as $key )
		{
			...
			} else if ( strpos($key, 'acf_') !== false && !in_array($key, $dont_delete) ) {
				// this is also a field, and it wasn't found in the dont_delete array
				do_action('acf/delete_field', $post_id, $key);
			}
		}
	...
	*/
	...
	/*************************************************************************************************
	 	...
	 	If my rewrite works then this function will no longer be necessary
	...
	*************************************************************************************************/
	...
	/*************************************
	 *
	 *  find_available_key( $key ) function.
	 *
	 *  @description: Sees if a given key is available then
	 *		iterates on it until finding an available one.
	 *  @since: 4.0.0
	 *  @created: 2013-02-13
	 *  @access public
	 *  @param string $key - original key to test
	 *  @return A key that is currently available in the databased
	 *  @author: Ben Huebscher
	 *
	 *************************************/

	function find_available_key( $key )
	{
		$itr = 1;

		$key = intval( get_option( 'find_available_key', 1 ) );
		...
		do {
			$itr++;
			$new_key = $key . '_' . $itr;
		} while ( !key_is_available( $new_key );
	...
	/***************************************************************
	 *
	 *  key_is_available( $key )  function.
	 *
	 *  @description: Tests if a given key has already been used.
	 *  @since: 4.0.0
	 *  @created: 2013-02-13
	 *  @access public
	 *  @param string $key - key to test
	 *  @return BOOL	=>		TRUE	Key is available.
	 *							FALSE	Key is unavailable.
	 *  @author: Ben Huebscher
	 *
	 ***************************************************************/

	function key_is_available( $key )
	{
		global $wpdb;
